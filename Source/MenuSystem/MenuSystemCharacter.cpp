	...
	JoinSessionCompleteDelegate(FOnJoinSessionCompleteDelegate::CreateUObject(this, &ThisClass::OnJoinSessionComplete))
{
	...
	if (OnlineSubsystem != NULL) {
		...
		//		15.0f, 
		//		FColor::Blue,
		//		FString::Printf(TEXT("Found subsystem name: %s"), *OnlineSubsystem->GetSubsystemName().ToString())
		//	);
		//}
	}
...
{
	// when pressed 1 -> Creates game session
	...
	// Actually set up the settings
	{
		SessionSettings->bIsLANMatch = false;
		SessionSettings->NumPublicConnections = 4;
		SessionSettings->bAllowJoinInProgress = true;
		SessionSettings->bAllowJoinViaPresence = true;
		SessionSettings->bShouldAdvertise = true; // let others know that session exists
	...
	if (bWasSuccessful == true) {
		if (GEngine) {
			GEngine->AddOnScreenDebugMessage(
				...
				15.0f,
				FColor::Green,
				FString::Printf(TEXT("Session Created! Session Name: %s"), *SessionName.ToString())
			);
		}

		// Also move to the lobby level
